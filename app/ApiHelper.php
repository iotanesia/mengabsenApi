...
class ApiHelper {
    ...
    static function createJwt($data = NULL, $is_refresh_token = FALSE) {
        ...
        // dd(env('JWT_SECRET'));
        return JWT::encode($payload, 'MENGABSENAPP','HS256');
    }
    ...
    static function getUserJwt($request, $is_refresh_token = FALSE) {
        ...
        $decoded_data = JWT::decode($token,new Key('MENGABSENAPP', 'HS256'));
    ...
    static function decodeJwt($token) {
        try {
            return JWT::decode($token,new Key('MENGABSENAPP', 'HS256'));
        } catch(\Throwable $e) {
            throw $e;
        }
